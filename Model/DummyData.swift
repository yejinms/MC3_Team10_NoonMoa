                ...
                if apt?.rooms.count == 1 {
                    ...
                    roomCounterRef.getDocument { (document, error) in
                        if let document = document, document.exists {
                            ...
                            let dummyUser = User(id: dummyUserId, roomId: newRoomId, aptId: aptId, state: State.active.rawValue, lastActiveDate: Date(), eyeColor: EyeColor.blue.rawValue, attendanceSheetId: nil, token: "")
