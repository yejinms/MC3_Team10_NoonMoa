...
public struct CalendarView<CalendarCell: View, HeaderCell: View>: View {
    ...
    public var body: some View {
        ...
        // MARK: - 달력 전체의 너비를 조절하는 변수
        var widthLevel : Float = 1.0
            ...
            InfinitePagerView(controller, orientation: controller.orientation) { yearMonth, i in
                LazyVGrid(columns: gridItem, alignment: .center, spacing: 0) {
                    ForEach(0..<(controller.columnCount * (controller.rowCount + (isHasHeader ? 1 : 0))), id: \.self) { j in
                        ...
                        // MARK: - 개별 요소마다 padding을 조절하는 변수
                        .padding(3)
                    }
                }
                .frame(width: proxy.size.width * CGFloat(widthLevel), height: proxy.size.height, alignment: .center)
            }
        }
    }
    ...
    func calculateCellHeight(_ index: Int, geometry: GeometryProxy) -> CGFloat {
        ...
        // MARK: - 개별 요소의 height를 조절하는 변수
        var heightLevel : Float = 0.45
        
        if !isHasHeader {
            return geometry.size.height / CGFloat(controller.rowCount) * CGFloat(heightLevel)
        }
        ...
        if index < controller.columnCount {
            ...
            return (geometry.size.height - headerHeight) / CGFloat(controller.rowCount) * CGFloat(heightLevel)
        }
    }
}
